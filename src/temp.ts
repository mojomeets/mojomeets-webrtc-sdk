...
export const startClient = async (credentials: ICredentials, localView?: HTMLDivElement, remoteView?: HTMLDivElement):Promise<void> => {
    ...
    if(credentials.role === 'MASTER'){
        ...
        client.signalingClient.on('open', async ():Promise<void> => {
            ...
            // This is responsible for sending video tracks 
            if (client.localStream) {
                client.localStream.getTracks().forEach((track:any) => client.peerConnection.addTrack(track, client.localStream));
            }
        ...
        client.signalingClient.on('sdpAnswer', async (answer:any):Promise<void> => {
        ...
        client.peerConnection.addEventListener('track', event => {
            ...
            remoteView!.srcObject = client.remoteStream;
        });
    }
...
export const stopClient = (credentials: ICredentials) => {
    ...
    if (client.localView) {
        client.localView.srcObject = null;
    }

    if (client.remoteView) {
        client.remoteView.srcObject = null;
    }
